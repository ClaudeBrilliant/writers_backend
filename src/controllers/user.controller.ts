...
import {
  ...
  deleteUser,
  updateUserBalance,
  updateUserBalanceAfterWithdraw
...
export const withdrawMoneyUserController = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
    const { decrementAmount } = req.body; // The amount to increment

    logger.info(`Updating user balance for ID: ${id}`);
    logger.debug(`Increment amount: ${decrementAmount}`);

    // Validate input
    if (typeof decrementAmount !== 'number' || decrementAmount <= 0) {
      logger.warn('Invalid increment amount provided');
      return res.status(400).json({ message: 'Invalid increment amount' });
    }

    // Call the service to update the balance
    const updatedUser = await updateUserBalanceAfterWithdraw(id, decrementAmount);
